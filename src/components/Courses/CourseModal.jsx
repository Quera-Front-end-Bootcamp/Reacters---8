            ...
            >
                <div className="modalContent relative bg-white w-full md:overflow-visible overflow-y-scroll h-4/5 md:h-fit">
                    ...
                    <nav className='w-full flex justify-between text-[#727272] p-5 text-sm border border-b-[#efefef]'>
                        ...
                        <p className='font-semibold text-base'>{data.title} دوره</p>
                    </nav>
                    {/* modalPhoto */}
                    <div className="modalPhoto flex flex-col items-center p-7 md:w-3/5 md:text-center md:float-left w-full float-none">
                        <img src={data.lesson.image} className="max-h-60" alt="coursePhoto" />
                    ...
                    <div className='flex md:justify-between justify-center items-center'>
                        <div dir="rtl" className="description flex flex-col md:items-start items-center text-gray-600 gap-5 p-8 md:float-left md:w-full md:border-l-2 float-none md:border-[#efefef] border-t">
                            <h4> مدرس : {data.teacher.fullName}</h4>
                            <h1>ظرفیت : {data.capacity.toLocaleString("fa")} نفر</h1>
                            <p>شروع : {new Date(data.startDate).toLocaleDateString('Fa-IR')}</p>
                            <p>پایان : {new Date(data.endDate).toLocaleDateString('Fa-IR')}</p>
                            <p>هزینه دوره : {data.cost.toLocaleString("fa")} تومان </p>
